...
<script language="javascript">
	...
	$(document).ready(function (){
		nodeID = "<%=nodeID%>";
		refID = "<%=refID%>";	//lessonID
		op = "<%=op%>";
		
		getComboBoxList("statusEffect","status",0);
		getComboList("coursewareID","coursewareInfo","coursewareID","coursewareName","status=0 order by ID",1);
		...
		$("#btnDel").linkbutton({
			...
			text:'移除',
			onClick:function() {
				jConfirm('你确定要移除这个课件吗?', '确认对话框', function(r) {
					if(r){
						$.get("lessonControl.asp?op=delLessonCoursewareNode&nodeID=" + $("#ID").val() + "&times=" + (new Date().getTime()),function(data){
							jAlert("移除成功！","信息提示");
							op = 1;
							setButton();
							updateCount += 1;
	...
	function getNodeInfo(id){
		$.get("lessonControl.asp?op=getLessonCoursewareNodeInfo&nodeID=" + id + "&times=" + (new Date().getTime()),function(re){
			//jAlert(unescape(re));
			var ar = new Array();
			...
			if(ar > ""){
				$("#ID").val(ar[0]);
				$("#lessonID").val(ar[1]);
				$("#coursewareID").combobox("setValue", ar[2]);
				$("#status").combobox("setValue", ar[4]);
				$("#seq").textbox("setValue", ar[6]);
				$("#proportion").textbox("setValue", ar[7]);
				$("#pages").textbox("setValue", ar[12]);
	...
	function saveNode(){
		//alert($("#lessonID").val() + "&memo=" + ($("#memo").val()));
		if($("#coursewareID").combobox("getValue")==""){
			$.messager.alert("提示","课件不能为空。","warning");
			return false;
		}
		$.get("lessonControl.asp?op=updateLessonCoursewareList&nodeID=" + $("#ID").val() + "&lessonID=" + $("#lessonID").val() + "&coursewareID=" + $("#coursewareID").combobox("getValue") + "&proportion=" + $("#proportion").textbox("getValue") + "&seq=" + $("#seq").textbox("getValue") + "&status=" + $("#status").combobox("getValue") + "&memo=" + escape($("#memo").textbox("getValue")) + "&times=" + (new Date().getTime()),function(re){
			//jAlert(re);
			if(re > 0){
				if(op == 1){
					op = 0;
	...
	function setEmpty(){
		$("#ID").val(0);
		$("#lessonID").val(refID);
		$("#vidoID").textbox("setValue", "");
		$("#pages").textbox("setValue", "0");
		$("#proportion").textbox("setValue", "100");
...
<div id='layout' align='left' style="background:#f0f0f0;">	
	...
	<div style="width:100%;float:left;margin:0;">
		<div style="border:solid 1px #e0e0e0;width:99%;margin:1px;background:#ffffff;line-height:18px;">
			...
			<table>
			<tr>
				<td align="right" style="width:70px;">序号</td><input id="ID" type="hidden" /><input id="lessonID" type="hidden" />
				<td><input id="seq" name="seq" class="easyui-textbox" data-options="height:22,width:100,required:true"></td>
				<td align="right" style="width:70px;">课件</td>
				<td><select id="coursewareID" name="coursewareID" class="easyui-combobox" data-options="panelHeight:'auto',height:22,width:260"></select></td>
			</tr>
			<tr>
				<td align="right">时长</td>
				<td><input id="pages" name="pages" class="easyui-textbox" data-options="height:22,width:100,readonly:true" />&nbsp;分钟</td>
				<td align="right">权重</td>
				<td><input id="proportion" name="proportion" class="easyui-textbox" data-options="height:22,width:100" />&nbsp;%</td>
			</tr>
			<tr>
