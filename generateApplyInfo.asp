	...
	$(document).ready(function (){
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人报名吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						...
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=" + reexamine + "&register=" + currUserName + "&host=" + currHost + "&classID=1&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
							type: "post",
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人上传报名表吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						...
						var start = performance.now(); 
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=9&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#applyID").textbox("getValue") + "&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
							type: "post",
							data: {"selList":selList},
							beforeSend: function() {   
								$.messager.progress();	// 显示进度条
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人上传个人证明吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						var start = performance.now(); 
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=9A&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#applyID").val() + "&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
							type: "post",
							data: {"selList":selList},
							beforeSend: function() {   
								$.messager.progress();	// 显示进度条
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人上传委托书吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						var start = performance.now(); 
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=9B&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#applyID").val() + "&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
							type: "post",
							data: {"selList":selList},
							beforeSend: function() {   
								$.messager.progress();	// 显示进度条
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人上传照片吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						var start = performance.now(); 
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=8&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#applyID").textbox("getValue") + "&courseName=" + $("#courseName").val() + "&reex=" + ((reexamine==0?"初证":"换证")),
							type: "post",
							data: {"selList":selList},
							beforeSend: function() {   
								$.messager.progress();	// 显示进度条
					...
					if(ar["msg"] > ""){
						...
						$.messager.confirm('确认对话框','确定要创建新的计划吗?<br>可能要花几分钟时间，请稍候...', function(r){
							if(r){
								var start = performance.now(); 
								$.ajax({
									url: uploadURL + "/public/applyEnter?SMS=1&reexamine=7&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#ID").val() + "&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
									type: "post",
			...
			onClick:function() {
				...
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人下载成绩和证书吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						...
						var start = performance.now(); 
						$.ajax({
							url: uploadURL + "/public/applyEnter?SMS=1&reexamine=10&register=" + currUserName + "&host=" + currHost + "&classID=" + $("#applyID").textbox("getValue") + "&courseName=" + $("#courseName").val() + "&reex=" + (reexamine==0?"初证":"换证"),
							type: "post",
		...
		$("#doApplyCheck").linkbutton({
			...
			onClick:function() {
				getSelCart("");
				if(selCount==0){
					$.messager.alert("提示","请选择要核查应换证日期的人员。","info");
					return false;
				}
				// jConfirm('确定要为这' + selCount + '个人报名吗?', "确认对话框",function(r){
				$.messager.confirm('确认对话框','确定要为这' + selCount + '个人核查应换证日期吗?<br>可能要花几分钟时间，请稍候...', function(r){
					if(r){
						var start = performance.now(); 
						$.ajax({
		...
		$.get("diplomaControl.asp?op=getApplyListByBatch&refID=" + nodeID + "&status=" + $("#s_status").combobox("getValue") + "&keyID=" + $("#s_resit").combobox("getValue") + "&wait=" + wait + "&upload=" + upload + "&needResit=" + need + "&needRetake=" + retake + "&closed=" + noclosed + "&times=" + (new Date().getTime()),function(data){
			...
			if(ar>""){
				...
				$.each(ar,function(iNum,val){
					...
					arr.push("<td class='link1' title='" + ar1[13] + "'><a href='javascript:showApplyInfo(" + ar1[0] + ",0,1);'>" + ar1[13].substring(0,6) + "</a></td>");
					h = "单位";
					...
					if(ar1[26]==''){
						...
						arr.push("<td class='center'><a href='javascript:void(0);' title='申报材料' onclick='showPic(\"" + ar1[26] + "\");' title='申报材料'>" + (ar1[37]>0?imgFileBlue:imgFile) + "</a></td>");
					}
...
<div id='layout' align='left' style="background:#f0f0f0;">	
	...
	<div style="width:100%;float:left;margin:10;height:4px;"></div>
  	<div class="buttonbox">
		...
		<a class="easyui-linkbutton" id="btnExamInfo" href="javascript:void(0)"></a>
  	</div>
	<div style="width:100%;float:left;margin:10;height:4px;"></div>
	<div class="buttonbox">
