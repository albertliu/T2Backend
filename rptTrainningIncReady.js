	...
	function getRptTrainningList(mark){
		...
		$.getJSON(uploadURL + "/public/getRptList?op=trainning&mark=" + mark + "&sales=" + $("#rptTrainningSales").combobox("getValue") + "&host=" + currHost + "&courseID=" + $("#rptTrainningCourseID").combobox("getValue") + "&startDate=" + $("#rptTrainningStartDate").val() + "&endDate=" + $("#rptTrainningEndDate").val() + "&mark1=" + mark1,function(data){
			//jAlert(data);
			if(data==""){
				jAlert("没有符合要求的数据。","提示")
			}
			...
			if(mark=="data" && data.length>0){
				...
				arr.push("<th width='8%'>日期</th>");
				arr.push("<th width='8%'>财付通G</th>");
				arr.push("<th width='8%'>财付通S</th>");
				arr.push("<th width='8%'>微信收款</th>");
				arr.push("<th width='8%'>其中在线</th>");
				arr.push("<th width='8%'>支付宝收款</th>");
				arr.push("<th width='8%'>其中在线</th>");
				arr.push("<th width='8%'>现金收款</th>");
				arr.push("<th width='8%'>对公汇款</th>");
				arr.push("<th width='8%'>小计</th>");
				arr.push("<th width='8%'>退款</th>");
				arr.push("<th width='8%'>合计</th>");
				arr.push("</tr>");
				arr.push("</thead>");
