...
if(op == "getCurrParam"){
	result = currDate + "|" + currUser + "|" + Session("name_key") + "|" + currYear + "|" + currMonth + "|" + currWeek + "|" + currHost + "|" + currPartner + "|" + getSysEnv("NODE_ENV_UPLOAD_URLS") + "|" + getSysEnv("NODE_ENV_BACKEND_ET") + "|" + currHostName;
	Response.Write(escape(result));
}
...
if(op == "checkExistKeyInTab"){
	result = 0;
	sql = "SELECT 1 as item from " + table + " where " + field + "='" + nodeID + "'";
	rs = conn.Execute(sql);
	if (!rs.EOF){
		result = rs("item").value;
	}
