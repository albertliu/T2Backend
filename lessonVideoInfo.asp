...
<script language="javascript">
	...
	$(document).ready(function (){
		...
		getComboList("videoID","videoInfo","videoID","videoName","status=0 order by ID",1);
		...
		$("#btnDel").linkbutton({
			...
			text:'移除',
			onClick:function() {
				jConfirm('你确定要移除这个视频吗?', '确认对话框', function(r) {
					if(r){
						$.get("lessonControl.asp?op=delLessonVideoNode&nodeID=" + $("#ID").val() + "&times=" + (new Date().getTime()),function(data){
							jAlert("移除成功！","信息提示");
							op = 1;
							setButton();
							updateCount += 1;
	...
	function getNodeInfo(id){
		$.get("lessonControl.asp?op=getLessonVideoNodeInfo&nodeID=" + id + "&times=" + (new Date().getTime()),function(re){
			//jAlert(unescape(re));
			var ar = new Array();
	...
	function saveNode(){
		...
		if($("#videoID").combobox("getValue")==""){
			$.messager.alert("提示","视频不能为空。","warning");
			return false;
		}
		$.get("lessonControl.asp?op=updateLessonVideoList&nodeID=" + $("#ID").val() + "&lessonID=" + $("#lessonID").val() + "&videoID=" + $("#videoID").combobox("getValue") + "&proportion=" + $("#proportion").textbox("getValue") + "&seq=" + $("#seq").textbox("getValue") + "&status=" + $("#status").combobox("getValue") + "&memo=" + escape($("#memo").textbox("getValue")) + "&times=" + (new Date().getTime()),function(re){
			//jAlert(re);
			if(re > 0){
				if(op == 1){
					op = 0;
	...
	<div style="width:100%;float:left;margin:0;">
		<div style="border:solid 1px #e0e0e0;width:99%;margin:1px;background:#ffffff;line-height:18px;">
			...
			<tr>
				...
				<td align="right" style="width:70px;">视频</td>
				<td><select id="videoID" name="videoID" class="easyui-combobox" data-options="panelHeight:'auto',height:22,width:260"></select></td>
			</tr>
			<tr>
				<td align="right">时长</td>
				<td><input id="minutes" name="minutes" class="easyui-textbox" data-options="height:22,width:80,readonly:true" />&nbsp;秒</td>
